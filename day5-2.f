...
C
   ...
   30 FORMAT(A,I8)
C
      WRITE(*,10)"Advent of Code 2024 day 5, part 2"
      WRITE(*,10)" "
C
...
C     page number is
      ...
      DO 100 WHILE (MANUAL(NUMPAGES).NE.0)
        NUMPAGES=NUMPAGES+1
  100 CONTINUE
C     Only want to add the middle page value of corrected manuals
C     for this part of the puzzle
      MIDVAL=0
C
C     For each page, check it obeys the ordering rules
      IF (.NOT. CHECK(MANUAL,NUMPAGES)) THEN
C     If not, we reorder and return the middle page value
        CALL REORDER(MANUAL,NUMPAGES)
...
C             has failed
  ...
  999 CONTINUE
      ...
      END
C
      SUBROUTINE REORDER(MANUAL,NUMPAGES)
      INTEGER MANUAL(*), NUMPAGES
C
      INTEGER IDX,TEMP
      LOGICAL CHECK
C
  100 CONTINUE
      IDX=2
      DO 200 WHILE(CHECK(MANUAL,IDX).AND.(IDX.LE.NUMPAGES))
        IDX=IDX+1
  200 CONTINUE
      IF (NUMPAGES.NE.IDX-1) THEN
C       This is a kind of bubblesort!
C       Not efficient, but effective for this size of dataset
        TEMP=MANUAL(IDX-1)
        MANUAL(IDX-1)=MANUAL(IDX)
        MANUAL(IDX)=TEMP
        GOTO 100
      ENDIF
      RETURN
