...
C
   ...
   20 FORMAT(A,I16)
C
      WRITE(*,10)"Advent of Code 2024 day 7, part 2"
      WRITE(*,10)" "
...
C     The number of possible operator configurations
      CONFIGS=3**(NUMVALUES-1)
C     Work through each configuration until we find one that
C     works, or we run out of configurations to test. Treat OPS
C     as base 3, 0 = add, 1 = mul, 2 = concat. As input values are
C     all less than 1000, we can simplify concat.
      DO 200 IDX=1,CONFIGS
        SUM=VALUES(1)
        OPS=IDX
        DO 100 JDX=1,NUMVALUES-1
          CHK=MOD(OPS,3)
          IF (CHK.EQ.0) SUM=SUM+VALUES(JDX+1)
          IF (CHK.EQ.1) SUM=SUM*VALUES(JDX+1)
          IF (CHK.EQ.2) THEN
            IF (VALUES(JDX+1).LT.10) THEN
              SUM=SUM*10+VALUES(JDX+1)
            ELSE
              IF (VALUES(JDX+1).LT.100) THEN
                SUM=SUM*100+VALUES(JDX+1)
              ELSE
                SUM=SUM*1000+VALUES(JDX+1)
              ENDIF
            ENDIF
          ENDIF
          OPS=OPS/3
...
C
  999 CONTINUE
C     Set TARGET=0 if couldn't use *, + or concat to find it
      IF (.NOT.HIT) TARGET=0
      RETURN
      END
