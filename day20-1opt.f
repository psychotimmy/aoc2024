...
C
      ...
      INTEGER TOTAL,ASTAR
      COMPLEX START,FINISH,ROUTE(19500),NEWROUTE(19500)
...
C     point cheating if new distance from the cheat to the end is less than
C     100 picoseconds less than not cheating. A cheat being
...
C
      DO L1=1,TARGET-99
C       Can't save at least 100 picoseconds if we're already close
C       to the target (also stops the later check going out of bounds)
...
C
        DO L2=1,8,2
          START=CHEAT(L2)
          CHECK=CHEAT(L2+1)
C         No point cheating if you end up further back on the path
C         after the cheat or less than 100 picoseconds further towards
C         the goal.
          DO L3=1,L1-1
            IF (START.EQ.ROUTE(L3)) GOTO 500
          ENDDO
          DO L3=L1+1,L1+99
            IF (START.EQ.ROUTE(L3)) GOTO 500
          ENDDO
