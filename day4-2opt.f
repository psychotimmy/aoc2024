...
C
      PARSE=0
C
C     Check the 4 directions for the sum M+S (A will always be there,
C     so no need to add in ) = 77+83 = 160
C 
      IF (((XMAS(L1-1,L2-1)+XMAS(L1+1,L2+1)).EQ.160) .AND.
     +    ((XMAS(L1+1,L2-1)+XMAS(L1-1,L2+1)).EQ.160)) THEN
        PARSE=1
      ENDIF
C
      RETURN
